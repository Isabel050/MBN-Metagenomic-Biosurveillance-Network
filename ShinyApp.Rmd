...
Slow=TRUE
#Slow=FALSE
if(Slow) { # comment out entire setup, in favor of loading image.
# This is good for loading speed, but we need to figure out what to do in production version.
...
if((file.exists("Master_Frame_a.xlsx")&file.exists("Master_Frame_Full.xlsx"))){ScenariosRun=TRUE} else {ScenariosRun=FALSE}
...
# Master_Frame_Full <- read.xlsx("Master_Frame_Full.xlsx") # Adds 10-15 seconds to loading!
...
- Master_Frame_Full is the same, for all percentages, to use for per-hospital selection
- Columns: HospPCT, Cases_continuous, DetectThreshold, Cumulative_probability
- Note: HospPCT can be mapped to Top N with Full_Cost_Map
...
Columns: Days, [Disease Name]


"}
save.image(file='ShinyApp_SetupRun.RData')
}
load('ShinyApp_SetupRun.RData')    
```
...
#Note: Moved Cost Calculation to other script.

n_Hosp_c=as.character(n_Hosp)
Overall_10y_Cost = Overall_10y_Cost_AnyNumber[n_Hosp] #Just the numbers needed.
